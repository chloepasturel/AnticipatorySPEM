...
%: this heuristic is an ideal observer it's a particular case of the solution to the generative model assumaing a fixed -block length
...
%: using \hat{p} as a regressor / limits of the leaky integrator / sufficient statistics of a beta distribution

only right once (switch at $\tau$) but best on average knowing the generative model

all bayesian models are right / some generative models are wrong - it is essential that these component can be controlled independently - are ideal observers wrong because of wrong approximations or because of a wrong model (see beck and pouget)
...
% --------------------------------------------------------------------------------------
%-------------------------------------------------------------%
%: overcoming this difficulty using a latent variable= run-length we will put in competition different leaky integrators in parallel + evazluate them / fundamental in bayesian methods / message passing
%-------------------------------------------------------------%
