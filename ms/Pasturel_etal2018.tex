...
\node [anchor=north west]  (img1) at (0.000\linewidth,.618\linewidth){\includegraphics[width=0.333\linewidth]{protocol_recording}};
...
% cf 0_protocole.ipynb
\node [anchor=north west]  (img3) at (0.666\linewidth,.618\linewidth){\includegraphics[width=0.25\linewidth]{protocol_bet_simple}};
...
by using random-length blocks to avoid potential inter-blocks confounds (\seeFig{results_psycho}).
...
a family of direction-biases for the target motion~(\seeFig{intro}-A).
...
The participants eye movements are recorded during
the observation of the sequence of pseudo-random direction trials~(\seeFig{intro}-A). % TODO : c'est pas dans l'autre sens?
What we found was a global and robust effect of direction-bias
on anticipatory smooth pursuit~(\seeFig{intro}-B).
...
linearly related to the probability of motion's speed or direction~(\seeFig{intro}-B, inset).
...
before target ramp-motion onset~\seeFig{intro}.
