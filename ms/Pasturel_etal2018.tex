...
the three levels of the stochastic model.
 ...
 %
~\textbf{B} Raw results for one characteristic observer.
...
volatility is modelled as the non-linear transformation
of a random walk (modelled itself by a Brownian motion).
...
to extract an inferrence of the agent's belief about volatility~\citep{Voessel??}.
...
and we denote this pariable as $x_0^t$ at trial $t$.
...
Then, we will present psychophysics on eye movements (reflex / cognitive) => generalization of previous results
...
the observation of the same sequence of random-direction trials~(\seeFig{intro}-A). % TODO : c'est pas dans l'autre sens?
In the fourth part, we will analyse the inferrence of the volatility
...
accross the two diferrent sessions testing predictive adaptive processes
at the uncouscious and conscious levels.
