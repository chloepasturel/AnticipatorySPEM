...
We applied the binomial switching model in a first session
to generate the (pseudo-) random sequence of
the dot's directions,
while we recordeed the particpants' eye movements.
Among our $12$ subjects, we show one representative example in~\seeFig{results_raw}
This representative participant was chosen as that
with the median fit in the quantitative analysis that will follow.
...
(as a green line, along with its confidence interval in a shaded area).
...
reflecting the time taken to integrate enough information
