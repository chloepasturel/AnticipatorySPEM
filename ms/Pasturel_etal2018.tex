...
before target ramp-motion onset~(\seeFig{intro}-A),
it is possible to bias the direction and mean velocity of aSPEM~(\seeFig{intro}-B).
...
any continuous value within the possible range (that is, $\[ 0, 1 \]$)
...
each trial being either the eye recording~(\seeFig{intro}-A) 
or in a subsequent day the rating scale~(\seeFig{intro}-C).
...
the bias in target direction is stationary between two events 
(that we denote as ``switches''),
and that it changes at the moment of a switch.
Mathematically, for each trial, a target moves either to the left or to the right,
and we denote at trial $t$ this variable (target direction) as $x_0^t$.
This direction is drawn from a Bernoulli process $\Bb$
with a parameter that we define as $p=x_1^t$.
One novelty is that the variable $p$ is itself a random variable:
...
x_1^t \eqdef p_t = p_{t-1} \quad \text{if} \quad s_t=0 \quad \text{and else} \quad p_t \propto \Pp \\
